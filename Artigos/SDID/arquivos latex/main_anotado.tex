...
% ── Anotações para versão revisada ───────────────────────
\usepackage[colorinlistoftodos,textsize=scriptsize]{todonotes}
\newcommand{\RA}[1]{\todo[color=blue!15,linecolor=blue!40,bordercolor=blue!40]{\textbf{Av.A:} #1}}
\newcommand{\RB}[1]{\todo[color=orange!20,linecolor=orange!60,bordercolor=orange!60]{\textbf{Av.B:} #1}}
...
\usepackage[
  ...
  left=3.00cm, right=5.50cm,
  headheight=1.5cm,
  headsep=0.5cm,
  footskip=1.2cm,
  marginparwidth=4.80cm
]{geometry}
...
\begin{center}
  ...
  {\normalsize Arthur Pereira Donato\footnote{arthurpdonato@gmail.com.
    Agradece o apoio financeiro da Coordenação de Aperfeiçoamento de Pessoal de
    Nível Superior -- Brasil (CAPES) -- Código de Financiamento 001.}}\\
  {\small Universidade Federal do Rio Grande (FURG), PPGE, Rio Grande, RS, Brasil}

  \vspace{0.5em}

  {\normalsize Alisson Tallys Geraldo Fiorentin\footnote{alisson.fiorentin@gmail.com.
    Agradece o apoio financeiro da Coordenação de Aperfeiçoamento de Pessoal de
    Nível Superior -- Brasil (CAPES) -- Código de Financiamento 001.}}\\
  ...
  {\normalsize Gibran da Silva Teixeira\footnote{tgibran@furg.com.}}\\
  {\small Universidade Federal do Rio Grande (FURG), PPGE, Rio Grande, RS, Brasil}

  \vspace{0.5em}

  {\normalsize Marcus Vinicius Bastos dos Santos\footnote{marcus.ssz@gmail.com.
    Agradece o apoio financeiro do Conselho Nacional de Desenvolvimento
    Científico e Tecnológico -- CNPq (Proc.\ 175091/2023-4).}}\\
  {\small Universidade Católica de Brasília (UCB), Brasília, DF, Brasil}

  \vspace{0.5em}

  {\normalsize Tiago Müller de Brito\footnote{ttiagobrito21@gmail.com.
    Agradece o apoio financeiro do Conselho Nacional de Desenvolvimento
    Científico e Tecnológico -- CNPq (Proc.\ a confirmar).}}\\
  {\small Universidade Federal do Rio Grande (FURG), PPGE, Rio Grande, RS, Brasil}
...
dos seis municípios analisados, gerando um déficit acumulado superior a 2.390
oportunidades de trabalho ao longo de onze meses. Esses resultados sustentam quatro
conclusões principais: (i) o desastre reduziu as contratações formais em quatro dos seis
municípios; (ii) a triangulação entre os três estimadores confirma convergência de sinal
e magnitude; (iii) o déficit acumulado supera 2.390 empregos formais ao longo de onze
meses; e (iv) a heterogeneidade entre municípios é mais bem explicada pelos canais de
dano produtivo do que pelo dano residencial. Os testes de placebo confirmam a
especificidade do choque às localidades afetadas, e as estimativas são consistentes com
estudos recentes baseados em Diferenças em Diferenças.
...
method as robustness exercises. The results reveal an immediate and severe shock to hiring in four of the six
municipalities analyzed, generating an accumulated deficit of more than 2,390
employment opportunities over eleven months. These findings support four main
conclusions: (i) the disaster severely reduced formal hiring in four of the six
municipalities; (ii) the triangulation of three estimators confirms convergence in sign
and magnitude; (iii) the accumulated deficit exceeds 2,390 formal jobs over eleven
months; and (iv) heterogeneity across municipalities is best explained by productive
damage channels rather than residential damage alone. Placebo tests confirm the
specificity of the shock to the affected localities, and the estimates are consistent
with recent Difference-in-Differences studies.
...
\RA{A.1 — Introdução integralmente reescrita: agora cita fontes precisas (Defesa Civil, IPEA), apresenta a metodologia (SDiD) e menciona a fonte de dados (CAGED). A.2 — Literatura conectada desde a abertura.}
...
Instituto de Pesquisa Econômica Aplicada (IPEA) apontam que, nos municípios mais
...
Nesse contexto, estimamos os efeitos causais das enchentes de maio de 2024 sobre as
admissões em empregos formais nos seis municípios gaúchos com mais de 50\% da população
atingida (Eldorado do Sul, Muçum, Roca Sales, Travesseiro, Arambaré e Igrejinha)
\cite{teixeira2024}, adotando o estimador \textit{Synthetic Difference-in-Differences} (SDiD),
...
de Empregados e Desempregados (CAGED).\RA{A.1 — Fonte de dados (CAGED) mencionada na introdução e no resumo/abstract, eliminando a lacuna identificada pelo avaliador.} Para assegurar a homogeneidade da série, a janela
...
cada município é construído a partir de um pool doador comum de municípios gaúchos
...
\label{sec:revisao}
\RA{A.2 — Revisão reestruturada em cinco blocos temáticos com literatura internacional sobre desastres climáticos, evidências para o RS e metodologia dos estimadores utilizados. Expandida em relação à versão original.}
...
pool doador de unidades comparáveis não afetadas. A aleatoriedade temporal condicional
das inundações fornece a base para assumir que o momento exato do evento é exógeno
condicional à exposição histórica ao risco, satisfazendo a condição de tratamento
exógeno requerida pelo SC \cite{elsner2024}.

\subsubsection{SDiD e Estimadores Híbridos em Avaliações de Desastres}
\RB{B.1.3 / B.2 — Subseção sobre SDiD na revisão posiciona o estimador na literatura de desastres e fundamenta sua adoção como estimador com grupo de controle externo (ponto central da crítica B.1).}
...
produzindo ajuste deficiente e estimativas do Efeito Médio do Tratamento sobre os
Tratados (ATT) potencialmente viesadas \cite{abadie2021}. O SDiD contorna esse
problema ao trabalhar em primeira diferença:
...
exclusão. No caso das enchentes de 2024, estimativas do Escritório de Coordenação de Assuntos
Humanitários das Nações Unidas (OCHA) apontaram contrações econômicas mensais de
36,3\%, 19,8\% e 19,3\% em Eldorado do Sul, Canoas e São Leopoldo, respectivamente,
frente ao mesmo mês do ano anterior \cite{ocha2024}.
...
\citeonline{deryugina2018} analisaram o impacto de longo prazo do furacão Katrina sobre
vítimas individuais com dados de declarações de imposto de renda e constataram que, após
a perturbação inicial, as que migraram para outras regiões experimentaram ganhos salariais
...
exogeneidade condicional necessária para identificação. O uso de imagens de radar de
...
componente que o caráter exógeno condicional das precipitações extremas de 2024 satisfaz:
...
\RB{B.3.3 / B.10.2 — Premissa de não-interferência (SUTVA) discutida analiticamente, não apenas declarada. A zona de exclusão donut ($>50\%$ tratados; $\leq 0{,}5\%$ doadores) é justificada aqui como mecanismo de mitigação. Limitação retomada nas Considerações Finais.}
...
autorregressivos espaciais ao SC para acomodar esses efeitos de transbordamento, embora a
...
\label{sub:lacunas}
\RB{B.1.3 — Subseção de lacunas posiciona o artigo frente à literatura e justifica por que o SDiD com grupo de controle externo é mais adequado do que abordagens univariadas sem contrafactual (cf.\ versão anterior deste trabalho baseada em C-ARIMA).}
...
\RA{A.3 — Seção metodológica inteiramente reescrita com apresentação formal dos três estimadores e suas premissas de identificação; parágrafos ``confusos e inconclusivos'' eliminados.}
\RB{B.2 — Equilíbrio restaurado: TWFE-DiD, SC e SDiD recebem subseções simétricas; a ênfase está no SDiD e na sua dupla robustez, não no ARIMA clássico.}
...
tendências paralelas: na ausência do tratamento, a trajetória
...
assume-se ausência de antecipação e consistência dos potenciais
resultados (SUTVA).
...
\RB{B.1.3 — SC incluído como exercício de triangulação, respondendo à sugestão de incluir ``ao menos um teste de robustez com grupo de controle externo''.}
...
denominado pool doador. Em vez de assumir tendências paralelas em nível, o método constrói
...
correspondente para o pool doador, e $V$ é uma matriz diagonal positiva
...
espaço: aplica-se o método a cada unidade do pool doador como se fosse a tratada e
...
unidade tratada, o que equivale a um teste de permutação com validade finita para
...
de controle; (iii) a seleção do pool doador e dos preditores envolve
discricionariedade do pesquisador; e (iv) os pesos são calculados apenas sobre o período
pré-tratamento, sem qualquer reponderação das janelas temporais
\cite{arkhangelsky2021}.

% ── 3.3 ──────────────────────────────────────────────────
\subsection{\textit{Synthetic Difference-in-Differences} (SDiD)}
\RB{B.1.1 / B.1.2 / B.1.3 — \textbf{Resposta central ao Avaliador B.} A adoção do SDiD como estimador principal substitui integralmente o C-ARIMA e constrói o contrafactual externo demandado: pool de 193 municípios com $\leq 0{,}5\%$ da população atingida. O estimador é duplamente robusto a desvios em cada conjunto de pesos.}
...
\RA{A.3 — Tabela comparativa dos estimadores introduzida para facilitar a leitura da seção metodológica.}
\RB{B.2 — Síntese comparativa garante apresentação equilibrada e contextualiza as vantagens de cada método.}
...
Premissa central & Tendências paralelas em nível &
  ...
  Modelo aditivo + fator latente; reponderação dupla \\[4pt]
...
entre as trajetórias pré-evento dos municípios atingidos e do pool doador.
...
\label{sub:variaveis}
\RA{A.1 — Fonte de dados (CAGED/Novo CAGED) descrita detalhadamente nesta subseção, com cobertura, obrigação legal e limitações.}
\RB{B.6 — Janela inicia em jan./2021 para excluir a pandemia e a quebra estrutural da transição para o Novo CAGED em 2020; substituindo a análise ``com e sem pandemia'' sugerida por uma solução estrutural no design.}
...
\label{sub:tratados}
\RB{B.4.1 — Critério de seleção ($>50\%$ da população atingida, fonte MUPRS/SPGG-RS) explicitado com justificativa substantiva e pré-especificação. Amostra ampliada de 2 para 6 municípios, com tabela permitindo verificação direta pelo leitor.}
...
\label{sub:pool}
\RB{B.3.3 / B.9 — Design donut: limiar de $\leq 0{,}5\%$ define a zona de exclusão que mitiga SUTVA. Os mesmos 193 municípios são usados para o teste de permutação direta, substituindo o kNN de seleção opaca da versão original.}
...
(listados no Apêndice~\ref{apx:pool_doador}).
...
transbordamentos produtivos) fornece evidência indireta de que a contaminação do
...
\label{sub:att}
\RA{A.5 / B.8 — Resultados apresentados sem atribuição causal a políticas específicas. A linguagem está restrita ao que o estimador efetivamente mede (ATT sobre admissões formais).}
\RB{B.7 — RMSPE pré-tratamento reportado e contextualizado para cada município; Muçum (RMSPE elevado) qualificado como inferência parcial.}
...
\item IC = Intervalo de Confiança a 95\% (por permutação direta).
...
\begin{figure}[htbp]
\centering
% ── Linha 1: Eldorado do Sul + Muçum ────────────────────────────────
\subcaptionbox{Eldorado do Sul\label{fig:traj_es}}
  [0.47\textwidth]{\includegraphics[width=0.47\textwidth]%
...
% ── Linha 2: Roca Sales + Travesseiro ────────────────────────────────
...
pool completo ($N_0 = 193$). Eixo horizontal: período (jan./2021–mar./2025);
eixo vertical: admissões formais por mil habitantes. Linha vertical:
mai./2024 (início do tratamento). \textbf{Linha azul:} controle sintético
SDiD; \textbf{linha laranja:} unidade tratada (observada). A área sombreada
inferior representa os pesos temporais $\hat{\lambda}_t$, concentrados pelo
SDiD nos meses imediatamente anteriores ao desastre \citeonline{arkhangelsky2021}.
Municípios com efeito robusto (IC95\% inteiramente negativo): Eldorado do
Sul, Roca Sales e Travesseiro. $^{\dagger}$RMSPE\,pré\,$> 5{,}0$ (inferência
parcial; ver Subseção~\ref{sub:robustez}). Fonte: elaboração própria.}
...
\label{sub:robustez}
\RB{B.9 — Placebo por permutação direta sobre os 193 doadores (sem kNN): cada doador é tratado ficticiamente e o p-valor é a proporção de placebos com $|\hat{\tau}| \geq |\hat{\tau}_{\text{real}}|$. Procedimento standard, transparente e replicável.}
...
afetados (Seção~\ref{sub:pool}). Por fim, a robustez à escolha da variável dependente
é avaliada no Apêndice~\ref{apx:saldo}, que replica o estudo principal utilizando o
saldo líquido mensal (admissões menos desligamentos) por mil habitantes como variável
de resposta, com resultados consistentes com os encontrados para as admissões.
...
\RB{B.1.3 — Triangulação TWFE-DiD, SC e SDiD implementa diretamente a recomendação de incluir estimadores com grupo de controle externo. Convergência de sinal e magnitude nos municípios robustos fortalece a identificação.}
\RA{A.3 — Os três estimadores da seção metodológica são aplicados aqui com comparação sistemática.}
...
e o SC produz a estimativa de menor magnitude ($-2{,}92$), coerente com a limitação do SC em
contextos de RMSPE pré elevado \cite{abadie2010}. O SDiD, ao incorporar pesos temporais ótimos,
acomoda parcialmente a má aderência pré-tratamento \cite{arkhangelsky2021}.
Em Arambaré e Igrejinha, todos os três estimadores produzem intervalos que
cruzam o zero, confirmando a ausência de efeito. A ``convergência'' discutida
refere-se à concordância de sinal e de ordem de grandeza, e não à significância
estatística — o SDiD tem maior potência por combinar rebalanceamento duplo
($\hat{w}_i$ e $\hat{\lambda}_t$) com permutação exata sobre os 193 placebos
\cite{arkhangelsky2021}.
...
IC95\% \textit{cluster-robust}. Linha vertical azul: $k = 0$ (mai./2024); referência
$\hat{\delta}_{-1} = 0$ por normalização. \textbf{Linha vermelha:} coeficientes
pós-tratamento ($k \geq 0$); \textbf{linha cinza:} coeficientes pré-tratamento
($k < 0$). Círculos cheios: IC95\% exclui zero. $N = 199$ municípios, $T = 51$
períodos. Fonte: elaboração própria.}
...
reponderação. Esse diagnóstico confirma, \textit{ex post}, a adequação do SDiD
...
Os efeitos acumulados para todos os municípios tratados encontram-se no
Apêndice~\ref{apx:acumulado}.
...
\RA{A.5 / B.8 — Análise de heterogeneidade e canais de dano explicitamente qualificada como \emph{descritiva e geradora de hipóteses}, sem atribuição causal a políticas. Nota metodológica esclarece os limites inferenciais com n=6 municípios.}
...
Cabe ressaltar que esta subseção é estritamente descritiva e geradora de hipóteses.
Com apenas seis unidades ($n=6$), não se conduz regressão nem inferência formal entre
municípios; a leitura é associativa, não causal no plano intermunicipal. Os auxílios
emergenciais (p.\,ex., Volta por Cima, SOS-RS) foram excluídos do conjunto de
covariáveis de dano por serem pós-tratamento e endógenos ao próprio nível de
destruição, configurando variáveis de controle inválidas \cite{angrist2009}.
...
A Figura~\ref{fig:canais} sintetiza os canais centrais de dano por município.
...
\RB{B.10.1 — Comparação com Teixeira et al.\ (2024) reposicionada como convergência de evidências, com reconhecimento explícito das diferenças de método, amostra e normalização.}
\RB{B.10.2 — Limitações metodológicas declaradas explicitamente: (i) emprego informal não capturado; (ii) heterogeneidade descritiva com n=6; (iii) janela encerrada antes de recuperação completa.}
\RA{A.5 / B.8 — Avaliação de políticas de recuperação posicionada como agenda de pesquisa futura, não como conclusão do modelo.}
...
Além das respostas de curto prazo ao choque, os resultados desta análise apontam
para uma agenda de resiliência estrutural voltada à redução da vulnerabilidade dos
municípios à recorrência de eventos climáticos extremos. A literatura sobre desastres
e desenvolvimento econômico converge em um diagnóstico central: municípios com
estruturas produtivas diversificadas, firmas capitalizadas, infraestrutura logística
redundante e mercados de trabalho integrados regionalmente recuperam-se mais
rapidamente e com menor perda permanente de bem-estar
\cite{botzen2019, kahn2005death, berlemann2018}. Os achados deste estudo são
coerentes com esse diagnóstico: os municípios com maior impacto detectável sobre o
emprego formal são, em geral, aqueles com tecido produtivo mais concentrado,
dependência de eixos viários únicos e menor integração com mercados vizinhos.

No plano produtivo, duas intervenções se mostram prioritárias. A primeira é a
diversificação da base econômica local: municípios com maior complexidade econômica
--- caracterizada por uma estrutura densa de vínculos inter-setoriais --- apresentam
maior capacidade de reabsorção de trabalhadores no pós-desastre, pois o colapso de
um setor não elimina as alternativas locais de emprego \cite{oclery2024}. A segunda
é o fortalecimento da capacidade de absorção das firmas: \citeonline{leiter2009}
documentam, a partir de dados de empresas europeias afetadas por inundações, que o
processo de recuperação do emprego é profundamente heterogêneo em função do nível de
capitalização --- empresas com acesso a crédito e reservas de liquidez tendem a
reinvestir e até expandir no pós-desastre, ao passo que micro e pequenas empresas
absorvem o choque de forma permanente. Mecanismos preventivos de capitalização ---
como seguros paramétricos indexados a indicadores físicos do desastre e linhas de
crédito emergencial pré-contratadas --- podem reduzir substancialmente a magnitude e
a persistência do impacto sobre o emprego formal, especialmente em economias locais
dominadas por firmas de menor porte, como as do Vale do Taquari
\cite{fatica2022, deryugina2017}.

No plano logístico e territorial, dois eixos complementares emergem dos resultados.
O primeiro é a melhoria da integração inter-regional dos mercados de trabalho:
\citeonline{deryugina2018} mostram que a mobilidade inter-regional no pós-desastre
pode gerar ganhos salariais persistentes para os trabalhadores realocados; os efeitos
de transbordamento documentados por \citeonline{lima2018} para desastres no Brasil
indicam que o isolamento geográfico amplifica os custos agregados do evento,
reforçando a importância de sistemas de informação sobre vagas em mercados vizinhos,
programas de requalificação profissional e instrumentos institucionais de suporte à
realocação. O segundo --- e possivelmente o de maior retorno esperado para as regiões
estudadas --- é o investimento em infraestrutura logística resiliente:
\citeonline{xiao2014} documentam que interrupções logísticas prolongam o desemprego
muito além do choque inicial, ao segmentar o mercado de trabalho local do restante da
rede produtiva regional. Para regiões de topografia restritiva como o Vale do
Taquari, onde a estreiteza dos vales torna toda a cadeia produtiva vulnerável ao
bloqueio de uma única estrutura viária, a construção de rotas alternativas de
escoamento, o reforço de pontes em pontos críticos e a ampliação da malha ferroviária
representam investimentos com elevado retorno esperado em termos de resiliência
econômica \cite{kousky2014, hallegatte2019}.
...
\label{apx:saldo}
\RB{B.5.1 — Apêndice criado inteiramente em resposta ao Avaliador B: replica a triangulação SDiD/SC/TWFE-DiD com o saldo líquido (admissões $-$ desligamentos) sobre o mesmo painel e pool doador. Resultados consistentes com o estudo principal.}
...
o saldo. As Figuras~\ref{fig:painel_adm_saldo_sig} e~\ref{fig:painel_adm_saldo_nsig}
exibem, para cada município, as trajetórias observada e sintética das duas variáveis
lado a lado --- separadas por grupo de resultado (significante vs.\ sem efeito detectável).
...
(TWFE-DiD, SC e SDiD; pool completo, $N_0 = 193$). \textbf{Cor:} vermelho = IC95\%
exclui zero; cinza = IC95\% cruza zero. \textbf{Forma (legenda no gráfico):}
círculo ($\bullet$) = inferência por permutação direta (SC e SDiD); triângulo
($\blacktriangle$) = IC gaussiano por EP \textit{cluster} (TWFE-DiD).
Fonte: elaboração própria.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{municipio_4306767_adm_vs_saldo.png}\\[0.5em]
\includegraphics[width=\textwidth]{municipio_4312609_adm_vs_saldo.png}\\[0.5em]
\includegraphics[width=\textwidth]{municipio_4315800_adm_vs_saldo.png}\\[0.5em]
\includegraphics[width=\textwidth]{municipio_4321626_adm_vs_saldo.png}
\caption{Trajetórias observada e controle sintético (SDiD) para admissões formais
(esquerda) e saldo líquido (direita): municípios com efeito significante (Eldorado
do Sul, Muçum, Roca Sales e Travesseiro). Linha vertical tracejada: maio/2024.
A convergência pré-evento ilustra a qualidade do ajuste; a divergência pós-mai/2024
captura o efeito estimado. Fonte: elaboração própria.}
\label{fig:painel_adm_saldo_sig}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{municipio_4300851_adm_vs_saldo.png}\\[0.5em]
\includegraphics[width=\textwidth]{municipio_4310108_adm_vs_saldo.png}
\caption{Trajetórias observada e controle sintético (SDiD) para admissões formais
(esquerda) e saldo líquido (direita): municípios sem efeito detectável (Arambaré e
Igrejinha). Linha vertical tracejada: maio/2024. A ausência de divergência sistemática
após mai/2024 é consistente com a ausência de efeito significante. Fonte: elaboração
própria.}
\label{fig:painel_adm_saldo_nsig}
\end{figure}

% =============================================================
\clearpage
\section{Efeito acumulado sobre admissões formais: todos os municípios tratados}
\label{apx:acumulado}

Esta seção apresenta o efeito acumulado sobre admissões formais por mil
habitantes ao longo dos onze meses pós-desastre (mai./2024–mar./2025) para
os seis municípios tratados. As bandas de IC$95\%$ são obtidas por placebo
na curva sobre os 193 doadores.

\begin{figure}[htbp]
\centering
% ── Linha 1 ─────────────────────────────────────────────────────────────
\subcaptionbox{Eldorado do Sul\label{fig:acum_es}}
  [0.47\textwidth]{\includegraphics[width=0.47\textwidth]%
  {municipio_4306767_completo_efeito_acumulado.png}}
\hfill
\subcaptionbox{Muçum\label{fig:acum_mucum}}
  [0.47\textwidth]{\includegraphics[width=0.47\textwidth]%
  {municipio_4312609_completo_efeito_acumulado.png}}

\vspace{0.6em}

% ── Linha 2 ─────────────────────────────────────────────────────────────
\subcaptionbox{Roca Sales\label{fig:acum_rs}}
  [0.47\textwidth]{\includegraphics[width=0.47\textwidth]%
  {municipio_4315800_completo_efeito_acumulado.png}}
\hfill
\subcaptionbox{Travesseiro\label{fig:acum_trav}}
  [0.47\textwidth]{\includegraphics[width=0.47\textwidth]%
  {municipio_4321626_completo_efeito_acumulado.png}}

\vspace{0.6em}

% ── Linha 3 ─────────────────────────────────────────────────────────────
\subcaptionbox{Arambaré\label{fig:acum_aram}}
  [0.47\textwidth]{\includegraphics[width=0.47\textwidth]%
  {municipio_4300851_completo_efeito_acumulado.png}}
\hfill
\subcaptionbox{Igrejinha\label{fig:acum_igrej}}
  [0.47\textwidth]{\includegraphics[width=0.47\textwidth]%
  {municipio_4310108_completo_efeito_acumulado.png}}

\caption{Efeito acumulado sobre admissões formais por mil habitantes
(mai./2024–mar./2025), por município tratado. Eixo horizontal: período
(mai./2024–mar./2025); eixo vertical: efeito acumulado em admissões
formais por mil habitantes. Banda laranja: IC$95\%$ por placebo na curva
sobre os $N_0 = 193$ doadores. Fonte: elaboração própria.}
\label{fig:acumulado_todos}
\end{figure}

% =============================================================
\clearpage
\section{Municípios do pool doador}
\label{apx:pool_doador}

Os 193 municípios gaúchos que compõem o pool doador --- selecionados pelo
critério de proporção de população atingida inferior a $0{,}5\%$ segundo o
MUPRS/SPGG-RS --- estão listados na Tabela~\ref{tab:pool_doador}.

\input{pool_doador_table.tex}
